...
site_coords <- readr::read_csv("./data/site_coordinates.csv") %>% 
...
year_plot2 <- epi_snp_clusters %>% 
    mutate(coded_site_num = as.numeric(sub("Site ", "", coded_site))) %>% 
    mutate(Region = factor(Region, levels=region_order)) %>% 
    arrange(factor(Region, levels=region_order), coded_site_num) %>%
...
ggsave(file=file.path(figs_dir, "FigureS2_GeotemporalDistrib.pdf"), 
       plot=year_region_plot, width=9, height=12, bg="white")
...
ggsave(file=file.path(figs_dir, "Figure1_ThresholdSensitivity.pdf"), 
       plot=sens_fig_manuscript, width=9, height=10)
```
...
ggsave(file=file.path(figs_dir, "FigureS3_ThresholdSensitivity1000SNPs.png"),
       plot=sens_fig_1000SNPs, width=10, height=10)
ggsave(file=file.path(figs_dir, "FigureS3_ThresholdSensitivity1000SNPs.pdf"),
       plot=sens_fig_1000SNPs, width=10, height=10)
```
...
plot_meta <- function(meta_obj, meta_df, events_lab, x_lab="Proportion") {
    ...
    p.left <- meta_df %>% 
        ...
        # Sites
        geom_text(aes(x=0, label = Site_ctry), hjust=0, size=4.5) +
        annotate("text", x=0, y=nrow(meta_df)+1, label="Site", hjust=0, fontface="bold", size=4.5) + 
        annotate("text", x=0, y=0, label="Random effects model", hjust=0, fontface="bold", size=4.5) +
        # N isolates
        geom_text(aes(x=2, label=`Total isolates`), hjust=0, size=4.5) +
        annotate("text", x=2, y=nrow(meta_df)+1, label="Total", hjust=0, fontface="bold", size=4.5) +
        annotate("text", x=2, y=0, label=sum(meta_obj$n), hjust=0, fontface="bold", size=4.5) +
        # N events
        geom_text(aes(x=2.5, label=Events), hjust=0, size=4.5) +
        annotate("text", x=2.5, y=nrow(meta_df)+1, label=events_lab, hjust=0, 
                 vjust=0.2, fontface="bold", size=4.5, lineheight = 0.75) +
        annotate("text", x=2.5, y=0, label=sum(meta_obj$event), hjust=0, fontface="bold", size=4.5) +
        # Prop
        geom_text(aes(x=3.3, label=sprintf("%.2f [%.2f, %.2f]", Proportion, lower_CI, upper_CI)), 
                  hjust=0, size=4.5) +
        annotate("text", x=3.3, y=nrow(meta_df)+1, label="Proportion\n[95% CI]", 
                 lineheight=0.75, vjust=0.2, hjust=0, fontface="bold", size=4.5) +
        annotate("text", x=3.3, y=0, 
                 label=sprintf("%.2f [%.2f, %.2f]", meta_df$pooled_Est[1], 
                               meta_df$pooled_lower[1], meta_df$pooled_upper[1]), 
                 hjust=0, fontface="bold", size=4.5) +
        # Heterogeneity
        ggtext::geom_richtext(aes(x=0, y = -1.3, label = heterogeneity_lab, family="sans"), 
                              data=meta_df[1,],
                              hjust=0, vjust = 0, size = 3.8, fill = NA, label.color = NA,
...
ggsave(file=file.path(figs_dir, "Figure2_PooledEstimates.png"), 
       plot=combined_forest_plots_sites_v, width=15, height=16, bg='white')
ggsave(file=file.path(figs_dir, "Figure2_PooledEstimates.pdf"), 
       plot=combined_forest_plots_sites_v, width=15, height=16, bg='white')
```
...
ggsave(file=file.path(figs_dir, "FigureS6a_PooledEstimatesRegion.png"), 
       plot=ggarrange(cluster_prop_meta_by_region_forest, labels=c("(A) Cluster proportion"),
                      font.label = list(size = 18)), 
       width=11, height=11, bg='white')
ggsave(file=file.path(figs_dir, "FigureS6a_PooledEstimatesRegion.pdf"), 
       plot=ggarrange(cluster_prop_meta_by_region_forest, labels=c("(A) Cluster proportion"),
                      font.label = list(size = 18)), 
       width=11, height=11, bg='white')
ggsave(file=file.path(figs_dir, "FigureS6b_PooledEstimatesRegion.png"), 
       plot=ggarrange(transmi_prop_meta_by_region_forest, labels=c("(B) Transmission proportion"),
                      font.label = list(size = 18)), 
       width=11, height=11, bg='white')
ggsave(file=file.path(figs_dir, "FigureS6b_PooledEstimatesRegion.pdf"), 
       plot=ggarrange(transmi_prop_meta_by_region_forest, labels=c("(B) Transmission proportion"),
                      font.label = list(size = 18)), 
       width=11, height=11, bg='white')
...
facilities_df <- read_tsv("./data/facilityInfo.tsv", show_col_types=F) %>% 
    ...
    arrange(Site)
write_tsv(facilities_df %>% select(
        Region, Country, Location, Facility_type, Facility_size, 
...
facs_clust_prop_boxplots_lst <- plot_clust_prop_by_facility_data(
    ...
    c("Facility_type", "Onsite_surgery", "Facility_size", "Piped_water", "No. neonatal beds"))
facs_clust_prop_boxplots <- (
    facs_clust_prop_boxplots_lst[[1]] | 
    facs_clust_prop_boxplots_lst[[2]] + theme(axis.title.y=element_blank(), axis.text.y=element_blank()) |  
    facs_clust_prop_boxplots_lst[[3]] + theme(axis.title.y=element_blank(), axis.text.y=element_blank()) |
    facs_clust_prop_boxplots_lst[[4]] | 
    facs_clust_prop_boxplots_lst[[5]] + theme(axis.title.y=element_blank(), axis.text.y=element_blank()) 
    ) + 
    plot_layout(nrow=2) +
    plot_annotation(tag_levels="A", tag_prefix="(", tag_suffix=")") &
    theme(plot.tag = element_text(size=16, face='bold'))

ggsave(file=file.path(figs_dir, "Figure3_ClusterPropByFacilityCharacteristics.png"),
       plot=facs_clust_prop_boxplots, width=12, height=10, bg='white')
ggsave(file=file.path(figs_dir, "Figure3_ClusterPropByFacilityCharacteristics.pdf"),
       plot=facs_clust_prop_boxplots, width=12, height=10, bg='white')


# Heatmap of site characteristics 
facs_heatmap_c <- plot_facility_data_heatmap(facilities_df, cluster_prop_meta_df)
facs_heatmap_t <- plot_facility_data_heatmap(facilities_df, transmi_prop_meta_df)
...
clust_meta_with_facility_heatmap <- (
        ...
        facs_heatmap_c$p1 | facs_heatmap_c$p2 | 
        facs_heatmap_c$p3 | facs_heatmap_c$p4 | 
        facs_heatmap_c$p5 + theme(legend.key.height = unit(6, "mm")) | 
        facs_heatmap_c$p6
    ) + 
    plot_annotation(
        title = "(A) Cluster proportion",
        theme = theme(plot.title = element_text(size=16, face="bold"), plot.title.position = "panel")) +
    plot_layout(widths=c(1,0.9,0.05,0.05,0.05,0.05,0.05,0.05), guides="collect") &
    theme(plot.tag = element_text(face="bold", vjust=-3.3),
          legend.position = "bottom",
          legend.direction = "vertical", legend.box = "horizontal")

ggsave(file=file.path(figs_dir, "FigureS7_PooledEstimatesWithFacilityDetails.png"),
       plot=clust_meta_with_facility_heatmap & theme(plot.title = element_blank()), 
       width=15, height=10, bg='white')
ggsave(file=file.path(figs_dir, "FigureS7_PooledEstimatesWithFacilityDetails.pdf"),
       plot=clust_meta_with_facility_heatmap & theme(plot.title = element_blank()), 
       width=15, height=10, bg='white')


transmi_meta_with_facility_heatmap <- (
        transmi_prop_meta_plots$p.left |
        transmi_prop_meta_plots$p.right + theme(legend.key.height = unit(2, "mm")) |
        facs_heatmap_t$p1 | facs_heatmap_t$p2 |
        facs_heatmap_t$p3 | facs_heatmap_t$p4 |
        facs_heatmap_t$p5 + theme(legend.key.height = unit(6, "mm")) |
        facs_heatmap_t$p6
    ) +
    plot_annotation(
        title = "(B) Transmission proportion",
        theme = theme(plot.title = element_text(size=16, face="bold"), plot.title.position = "panel")) +
    plot_layout(widths=c(1,0.9,0.05,0.05,0.05,0.05,0.05,0.05), guides="collect") &
    theme(plot.tag = element_text(face="bold", vjust=-3.3),
          legend.position = "none")
    
meta_plots_with_facility_heatmap <- 
    ggarrange(clust_meta_with_facility_heatmap, transmi_meta_with_facility_heatmap,
...
# Meta regression
covariates = c("Location", "Facility_type", "Onsite_surgery", 
               "Facility_size", "Neonatal_beds", "Piped_water")
...
# )
# all_cluster_plots
# ggsave(file=file.path(figs_dir, "FigureS5_ClustersPerSite.png"), 
#        plot=all_cluster_plots, width=15, height=16, bg="white")
# ggsave(file=file.path(figs_dir, "FigureS5_ClustersPerSite.pdf"), 
#        plot=all_cluster_plots, width=15, height=16, bg="white")
...
intros_vs_clusters_plot <- intros_vs_clusters %>% 
    ...
    ggrepel::geom_text_repel(
        ...
        aes(label=common_ST, color = signif), max.overlaps=40, size=5, seed=5) +
...
ggsave(file=file.path(figs_dir, "Figure5_TransmissionPredictors.png"),
       plot=trs_model_and_intros_plot, width=12, height=20)
ggsave(file=file.path(figs_dir, "Figure5_TransmissionPredictors.pdf"),
       plot=trs_model_and_intros_plot, width=12, height=20)
...
# ggsave(file=file.path(figs_dir, "FigureSX_ThresholdSensitivityST307.pdf"),
...
top_clones_combined_plot <-
    ...
    plot_annotation(
        ...
        subtitle = 'STs in ≥3 clusters & ≥2 sites',
...
ggsave(file=file.path(figs_dir, "Figure4_TransmissionClones.svg"), 
       plot=top_clones_combined_plot3, width=15, height=12) 
```
...
res_score_df <- epi_snp_clusters_wt_upd_STS %>% 
    ...
    select(id, Cluster, Study, Country, m_Site, Country_Study_Site, coded_site, 
...
ggsave(file=file.path(figs_dir, "FigureS8_Resistance_byCaseType.png"), 
       plot=res_case_type_plot, width=10, height=11)
ggsave(file=file.path(figs_dir, "FigureS8_Resistance_byCaseType.pdf"), 
       plot=res_case_type_plot, width=16, height=13)
ggsave(file=file.path(figs_dir, "FigureS8_Resistance_byCaseType.svg"), 
       plot=res_case_type_plot, width=16, height=13)
res_case_type_plot
...
ggsave(file=file.path(figs_dir, "FigureS4_DistanceMethodSensitivity.png"), 
       plot=dist_method_sens_plots, width=12, height=10, bg="white")
ggsave(file=file.path(figs_dir, "FigureS4_DistanceMethodSensitivity.pdf"), 
