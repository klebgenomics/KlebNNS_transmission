...
library(ggnetwork) # version 0.5.13
library(igraph) # version 2.0.3
library(future) # version 1.33.0
library(meta) # version 8.0.2
library(metafor) # version 4.8.0
library(grid) # version 4.2.3
library(gtable) # version 0.3.6
...
res_colors <- c(`0: ESBL-, Carb-` ="#FEE0D2", 
                ...
                `2: Carb+`        ="#EF3B2C", 
                `3: Carb+, Col+`  ="#99000D")
# res_colors <- c(`0: ESBL-, Carb-` ="#EFF3FF", 
#                 `1: ESBL+, Carb-` ="#9ECAE1", 
#                 `2: Carb+`        ="#4292C6", 
#                 `3: Carb+, Col+`  ="#084594")
...
get_metaprop_estimates <- function(meta_obj,
                                   prop_type=c("prop", "percent"), decimals=2) {
    ...
    estimates <- data.frame(
        S = meta_obj$studlab,
        Proportion = round(meta::backtransf(meta_obj$TE, sm=trns, n=n)*mult, decimals),  
        lower_CI = round(meta::backtransf(meta_obj$lower, sm=trns, n=n)*mult, decimals), 
        upper_CI = round(meta::backtransf(meta_obj$upper, sm=trns, n=n)*mult, decimals),
        pooled_Est = round(meta::backtransf(meta_obj$TE.random, sm=trns, n=n)*mult, decimals),
        pooled_lower = round(meta::backtransf(meta_obj$lower.random, sm=trns, n=n)*mult, decimals),
        pooled_upper = round(meta::backtransf(meta_obj$upper.random, sm=trns, n=n)*mult, decimals)
    ) %>% as_tibble() %>% arrange(desc(Proportion))
    return(estimates)
}

get_simple_estimates <- function(all_sites_clusters_summary_df, 
                                 group_column="site",
                                 total_column="n_isolates",
                                 events_column=c("n_isolates_clusters", "n_transmission_isolates"), 
                                 prop_type=c("prop", "percent"), decimals=2) {
    events_column = match.arg(events_column)
    prop_type = match.arg(prop_type)
    if (prop_type == "prop") {mult <- 1} else {mult <- 100}
    
    d <- all_sites_clusters_summary_df %>% select(
        S = !!sym(group_column),
        Total = !!sym(total_column),
        Events = !!sym(events_column),
        Region, Country)
    
    pooled <- broom::tidy(binom.test(sum(d$Events),  sum(d$Total)))

    estimates <- d %>% 
        rowwise() %>% 
        mutate(test = list(broom::tidy(binom.test(Events,Total)))) %>% 
        tidyr::unnest(test) %>% 
        ungroup() %>% 
        mutate(pooled_Est=pooled$estimate, pooled_lower=pooled$conf.low, pooled_upper=pooled$conf.high) %>% 
        select(all_of(names(d)), Proportion=estimate, lower_CI=conf.low, upper_CI=conf.high,
               pooled_Est, pooled_lower, pooled_upper) %>% 
        mutate(across(c(Proportion, lower_CI, upper_CI, pooled_Est, pooled_lower, pooled_upper), 
                      \(x) round(x*mult, decimals))) %>% 
        arrange(desc(Proportion))
    
    estimates$median_Est <- median(estimates$Proportion)
...
cluster_prop_meta_plots <- plot_forest_meta(cluster_prop_meta, cluster_prop_meta_df, 
                                      "Cluster\nisolates", "Cluster proportion")
cluster_prop_forest_meta <- ggarrange(plotlist = cluster_prop_meta_plots, 
                                      ncol = 2, widths=c(1.8,2),
                                      align="h", legend="bottom", common.legend=T)
# cluster_prop_forest_meta
# ggsave(file=file.path(figs_dir, "meta_cluster_prop.png"),
#        plot=cluster_prop_forest_meta, width=14, height=8, bg='white')


#################################################################
# User simple estimates instead, no meta-analysis
cluster_prop_simple_df <- get_simple_estimates(all_sites_clusters_summary, events_column = "n_isolates_clusters")
cluster_prop_simple_plots <- plot_forest_simple(cluster_prop_simple_df, "Cluster\nisolates", "Cluster proportion")
cluster_prop_forest_simple <- ggarrange(plotlist = cluster_prop_simple_plots,
                                        ncol=2, widths=c(1.8,2), align="h", legend="bottom", common.legend=T)
cluster_prop_forest_simple
```
...
transmi_prop_meta_plots <- plot_forest_meta(transmi_prop_meta, transmi_prop_meta_df, 
                                     "Transmitted\nisolates", "Transmission proportion")
transmi_prop_forest_meta <- ggarrange(plotlist = transmi_prop_meta_plots, 
                                      ncol = 2, widths=c(1.8,2),
                                      align="h", legend="bottom", common.legend=T)
transmi_prop_forest_meta
# ggsave(file=file.path(figs_dir, "meta_transmission_prop.png"), 
#        plot=transmi_prop_forest_meta, width=14, height=8, bg='white')

transmi_meta_report <- 
    get_metaprop_estimates(transmi_prop_meta, prop_type = "percent", decimals=1) %>% 
...
#################################################################
# User simple estimates instead, no meta-analysis
transmi_prop_simple_df <- get_simple_estimates(all_sites_clusters_summary, events_column = "n_transmission_isolates")
transmi_prop_simple_plots <- plot_forest_simple(transmi_prop_simple_df, "Transmitted\nisolates", "Transmission proportion")
transmi_prop_forest_simple <- ggarrange(plotlist = transmi_prop_simple_plots,
                                        ncol=2, widths=c(1.8,2), align="h", legend="bottom", common.legend=T)
transmi_prop_forest_simple


#########################################
transmi_simple_report <- 
    get_simple_estimates(all_sites_clusters_summary, events_column = "n_transmission_isolates",
                         prop_type = "percent", decimals = 0) %>% 
    mutate(report=paste(
        glue("{S} (n = {Events}/{Total};"),
        glue("{Proportion}% [{lower_CI}-{upper_CI}%])")
    )) 
print(paste(
    glue("In {transmi_simple_report %>% filter(Proportion >= 50) %>% nrow()}"),
    glue("of the {length(unique(epi_snp_clusters$Site))} sites, at least half"),
    glue("of all cases were attributable to transmission. These included"),
    paste0(
        sub("(.*),", "\\1, and", 
            paste0(transmi_simple_report %>% filter(Proportion>=50) %>% pull(report), 
                   collapse = ", ")),
        "."),
    "Overall, at least a third of cases were attributable to transmission in",
    transmi_simple_report %>% filter(Proportion >= 33.3) %>% nrow(), "sites."
    ))
```


```{r pooled_estimates_figures_manuscript}
# # Using meta-analysis estimates
# combined_forest_plots_sites_v <- cluster_prop_forest_meta +
...
combined_forest_plots_sites_v <- cluster_prop_forest_simple +
  ggtitle("(A) Cluster proportion") + 
  theme(plot.title.position = "plot", 
        plot.title = element_text(hjust = 0, size = 12, face = "bold",
                                  margin = margin(b = 5))) + 
...
# median pcts 
get_simple_estimates(all_sites_clusters_summary, events_column="n_transmission_isolates", 
                     prop_type="percent", decimals=1)$median_Est[1]
get_simple_estimates(all_sites_clusters_summary, events_column="n_isolates_clusters", 
                     prop_type="percent", decimals=1)$median_Est[1]
```
...
```{r region_estimates}
get_simple_estimates(all_sites_clusters_summary, events_column="n_isolates_clusters", 
                     prop_type="percent", decimals=1) %>% 
    group_by(Region) %>% 
    reframe(Mean = mean(Proportion), 
            Median = median(Proportion),
            median_range = paste0(
                median(Proportion), "% [", paste0(range(Proportion), collapse="-"), "%]")
            ) %>% 
    arrange(desc(Median))
```
...
ggsave(file=file.path(figs_dir, "FigureS5a_PooledEstimatesWithFacilityDetails.png"),
       plot=clust_meta_with_facility_heatmap, width=8.5, height=6.5, bg='white')
...
# Rename LV STs (only those specifically forming clusters with the ST) to the predominant ST
clusters_STs <- epi_snp_clusters %>% filter(!is.na(Cluster)) %>% 
    group_by(Cluster) %>% count(ST) %>% select(-n)
dissim_ST_clust <- bind_rows(clusters_STs[duplicated(clusters_STs$Cluster),], 
                             clusters_STs[duplicated(clusters_STs$Cluster, fromLast=T),]) %>% 
    arrange(Cluster, ST) %>% 
    group_by(Cluster) %>% 
    mutate(STs = paste0(ST, collapse = "; ")) %>% 
    rowwise() %>% 
...
trs_model_AmrSite_df <- parse_logistf_model(trs_model_AmrSite) %>% clean_model_df()

# AMR + Site + ST
trs_model_AmrSiteSt <- logistf::logistf("Case ~ ESBL + CP + Site + commonST_", data=trs_pred_data)
trs_model_AmrSiteSt_df <- parse_logistf_model(trs_model_AmrSiteSt) %>% clean_model_df()

# AMR + ST
trs_model_AmrSt <- logistf::logistf("Case ~ ESBL + CP + commonST_", data=trs_pred_data)
trs_model_AmrSt_df <- parse_logistf_model(trs_model_AmrSt) %>% clean_model_df()

# ST
trs_model_St <- logistf::logistf("Case ~ commonST_", data=trs_pred_data)
trs_model_St_df <- parse_logistf_model(trs_model_St) %>% clean_model_df()

# ST + Site
trs_model_StSite <- logistf::logistf("Case ~ commonST_ + Site", data=trs_pred_data)
trs_model_StSite_df <- parse_logistf_model(trs_model_StSite) %>% clean_model_df()
...
trs_model_AmrK <- logistf::logistf("Case ~ ESBL + CP + commonKL_", data=trs_pred_data)
trs_model_AmrSiteK <- logistf::logistf("Case ~ ESBL + CP + Site + commonKL_", data=trs_pred_data)
trs_model_AmrStK <- logistf::logistf("Case ~ ESBL + CP + commonST_ + commonKL_", data=trs_pred_data)
trs_model_AmrSiteStK <- logistf::logistf("Case ~ ESBL + CP + Site + commonST_ + commonKL_", data=trs_pred_data)
...
anova(trs_model_AmrSite, trs_model_AmrSiteK) # Does adding KL improve model with AMR + Site
anova(trs_model_AmrSt, trs_model_AmrStK) # Does adding KL improve model with AMR + ST 
...
# K model dfs
trs_model_AmrK_df <- parse_logistf_model(trs_model_AmrK) %>% clean_model_df()
trs_model_AmrSiteK_df <- parse_logistf_model(trs_model_AmrSiteK) %>% clean_model_df()
trs_model_AmrStK_df <- parse_logistf_model(trs_model_AmrStK) %>% clean_model_df()
trs_model_AmrSiteStK_df <- parse_logistf_model(trs_model_AmrSiteStK) %>% clean_model_df()
...
trs_model_AmrO <- logistf::logistf("Case ~ ESBL + CP + O_type_grp", data=trs_pred_data)
trs_model_AmrSiteO <- logistf::logistf("Case ~ ESBL + CP + Site + O_type_grp", data=trs_pred_data)
trs_model_AmrStO <- logistf::logistf("Case ~ ESBL + CP + commonST_ + O_type_grp", data=trs_pred_data)
trs_model_AmrSiteStO <- logistf::logistf("Case ~ ESBL + CP + Site + commonST_ + O_type_grp", data=trs_pred_data)
...
anova(trs_model_AmrSite, trs_model_AmrSiteO) # Does adding O types improve model with AMR + ST + Site
anova(trs_model_AmrSt, trs_model_AmrStO) # Does adding O types improve model with AMR + ST + Site
...
# O model dfs
trs_model_AmrO_df <- parse_logistf_model(trs_model_AmrO) %>% clean_model_df()
trs_model_AmrSiteO_df <- parse_logistf_model(trs_model_AmrSiteO) %>% clean_model_df()
trs_model_AmrStO_df <- parse_logistf_model(trs_model_AmrStO) %>% clean_model_df()
trs_model_AmrSiteStO_df <- parse_logistf_model(trs_model_AmrSiteStO) %>% clean_model_df()
...
trs_model_AmrSiteStK_df %>% # start with and full-join largest/non-nested models
    ...
    left_join( # -
        trs_model_AmrK_df %>% 
            mutate(Eff = sprintf("%.1f [%.1f,%.1f]; p=%.4f", OR,lower,upper,P)) %>% 
            select(Predictor, "AMR + K" = Eff),
        by = "Predictor") %>% 
    left_join( # -
        trs_model_AmrSiteK_df %>% 
            mutate(Eff = sprintf("%.1f [%.1f,%.1f]; p=%.4f", OR,lower,upper,P)) %>% 
            select(Predictor, "AMR + Site + K" = Eff),
        by = "Predictor") %>% 
    left_join( # -
        trs_model_AmrStK_df %>% 
            mutate(Eff = sprintf("%.1f [%.1f,%.1f]; p=%.4f", OR,lower,upper,P)) %>% 
            select(Predictor, "AMR + ST + K" = Eff),
        by = "Predictor") %>% 
    left_join( # -
        trs_model_AmrO_df %>% 
            mutate(Eff = sprintf("%.1f [%.1f,%.1f]; p=%.4f", OR,lower,upper,P)) %>% 
            select(Predictor, "AMR + O" = Eff),
        by = "Predictor") %>% 
    left_join( # -
        trs_model_AmrSiteO_df %>% 
            mutate(Eff = sprintf("%.1f [%.1f,%.1f]; p=%.4f", OR,lower,upper,P)) %>% 
            select(Predictor, "AMR + Site + O" = Eff),
        by = "Predictor") %>% 
    left_join( # -
        trs_model_AmrStO_df %>% 
            mutate(Eff = sprintf("%.1f [%.1f,%.1f]; p=%.4f", OR,lower,upper,P)) %>% 
            select(Predictor, "AMR + ST + O" = Eff),
        by = "Predictor") %>% 
    select(Predictor, 
           "AMR", "AMR + Site", "AMR + Site + ST", 
           "AMR + Site + ST + K", "AMR + K", "AMR + Site + K", "AMR + ST + K", 
           "AMR + Site + ST + O", "AMR + O", "AMR + Site + O", "AMR + ST + O") %>% # view() # subset to report 
    write_tsv(., file.path(tables_dir, "TableS7_TransmissionPredictors.tsv"), na="-")
...
########################################################################################
ST_signif_colors <- setNames(
    c("#750000", "#FF8A8A", "grey70"),
    c("Common ST, p<0.05", "Common ST, p≥0.05", "Other STs")
)
...
intros_vs_clusters_plot <- intros_vs_clusters %>% 
    ...
    scale_color_manual(values = ST_signif_colors, name="ST") +
...
intros_clusters_frac_distrib_plot <- intros_vs_clusters %>% 
    ...
    geom_histogram(aes(fill = common_ST), binwidth = 0.05, color="black", linewidth=0.25) +
...
# Plot distribution of fractions of clusters/introductions (density plot)
intros_clusters_frac_distrib_plot_2 <- intros_vs_clusters %>% 
    ggplot(aes(x = pct_clustered/100)) +
    geom_density(aes(fill=signif), alpha=0.8) + 
    scale_fill_manual(values = ST_signif_colors, guide="none") +
    labs(x = "Fraction of introductions resulting in clusters", y = "Density") +
    theme_bw() + custom_plots_theme +
    theme(axis.text=element_text(size=12), axis.title = element_text(size=14),
          plot.title = element_text(size=14))
...
ESBL_CP_frac_plot <- epi_snp_clusters_dereplicated %>% 
    ...
    scale_fill_manual(values = ST_signif_colors, name="ST",
...
intros_clusters_combined_plot <- (
    intros_clusters_frac_distrib_plot_2 + 
    intros_vs_clusters_plot + 
    ESBL_CP_frac_plot
    ) + 
    plot_layout(design = c(area(1,1,4,8), area(5,1,9,4), area(5,5,9,8))) +
...
# Summary
combined_ska_pw_sens_df %>% # cluster prop
...
combined_ska_pw_sens_df %>% # transmission prop
    filter(distance_threshold %in% c(10), temporal_threshold %in% c(2,4,8)) %>%
    mutate(temporal_threshold = paste0("temp_",temporal_threshold)) %>% 
